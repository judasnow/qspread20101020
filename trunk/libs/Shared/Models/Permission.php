...
class Models_Permission extends Vi_Model
{ 
    /**
    ...
    public function getExpandPermission($groupId, $access, $per)
    {
        ...
        $select = " SELECT  `ep`.{$per['expand_table_id']} AS expand_table_id_value, `ep`.{$per['expand_display_name']} AS expand_display_name_value,  `p` . * , gp.*
                    ...
                    JOIN (SELECT * FROM `vi_permission` WHERE name = " . $this->getAdapter()->quote($per['name']) . " AND module = " . $this->getAdapter()->quote($per['module']) . ") AS  `p`
                    ON true
                    LEFT JOIN `vi_group_permission` AS  `gp`  
