...
class Models_User extends Vi_Model
{ 
    /**
    ...
    public function deleteUsers($userIds = array())
    {
        ...
        try {
            ...
            return Vi_Language::translate('Can not delete users now');
        }
    }
    /**
    ...
    public function getUserWithExpand($userId)
    {
        $select = $this->select()
                ->setIntegrityCheck(false)
                ->from(array('u' => $this->_name))
                ->join(array('ue' => $this->_prefix . 'user_expand'), 'u.user_id = ue.user_id')
    ...
    {
        return $this->getByColumnName(array('username'=>$userName))->current();
    }
    
    public function getByUserId($userId)
    {
        return $this->getByColumnName(array('user_id'=>$userId))->current();
    }
        ...
        if (! in_array('full_name', $exclude)) {
            if (array_key_exists('full_name', $user)) {
                if (null == $user['full_name']) {
                    $errors['full_name'] = Vi_Language::translate('Full name is required');
                }
            } else {
                throw new Exception('Repeat password is required in create new user form');
            }
    ...
    {
        if (null == $username) {
            return Vi_Language::translate('Username is required');
        }
        $rowSet = $this->getByColumnName(array('username' => $username));
        if (count($rowSet) > 0) {
            return Vi_Language::translate('Username is used. Please choose another name');
        } else {
            return true;
        }
    ...
    public function validateEmail($email)
    {
        ...
        if ($validator->isValid($email)) {
            $rowSet = $this->getByColumnName(array('email' => $email));
            if (count($rowSet) > 0) {
                return Vi_Language::translate('Email is used. Please use another email');
            }
            return true;
        } else {
            return Vi_Language::translate('Email is not correct');
        }
    }
    /**
     * Validate password
     ...
     * @return true|string  True if password is more than or equal the Vi_Constant::USER_PASSWORD_MIN_LENGTH.
     *                      String of error in others.
     */
    public function validatePassword($password)
    {
        if (strlen($password) >= Vi_Constant::USER_PASSWORD_MIN_LENGTH) {
            return true;
        } else {
            return Vi_Language::translate('Password length is more than ' 
                                 . (Vi_Constant::USER_PASSWORD_MIN_LENGTH - 1) . ' characters');
        }
    }
    /**
    ...
    public function validateRetypePassword($password, $retypePassword)
    {
        if ($password === $retypePassword) {
            ...
            return Vi_Language::translate('Password is not matched');
        }
    }
    /**
    ...
    public function sendForgotPasswordMail($userName)
    {
        ...
        $user   = $this->fetchRow(array(
                                            'username=?' => $userName,
                                            'enabled=?'   => Vi_Constant::STATUS_ENABLED
                                        ));
        if (null == $user) {
            return false;
        }
        ...
        $mail->view->link     = 'http://' . $_SERVER['HTTP_HOST'] . Vi_Registry::getAppBaseUrl()
